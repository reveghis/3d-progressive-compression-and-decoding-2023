...
function init() {
    ...
    let url = (document.URL.split('?')[1] || "output/icosphere_reset_color.obja");
    // Ensure path is absolute so it resolves from server root, not from /viewer/
    if (url && !url.startsWith('/') && !url.startsWith('http')) {
        url = '/' + url;
    }

    loader = new Loader(url, 65536, 0);
